...
成功記録を保存した後、公開中のAPI・bridgeのdigestとS3の配布物を照合し、古い`releases/<SHA-256>/worker.tar.gz`の全バージョンを削除する。
現在の配布物は1バージョンだけ残す。
整理に失敗しても配布の成功記録は維持し、`state.json`に`releasePruneError`を残す。
認証や一時的なAWSエラーを解消した後は、同じ実行ディレクトリで`python3 judge/rollout.py prune --config "$JUDGE_RELEASE_RUN/config.json" --run-dir "$JUDGE_RELEASE_RUN"`を実行して再試行する。
...
成功後に古いS3配布物は残らないため、旧版への復旧ではホストの退避物またはスナップショットを使用する。
