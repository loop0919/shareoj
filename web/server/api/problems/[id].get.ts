...
export default defineEventHandler(async event => {
  ...
  try { content = await publicContent(event, `/problems/${id}`) }
    ...
    return {
      id, difficultyAverage: null, difficultyVoteCount: 0, difficulty: draft.difficulty, favoriteCount: 0, title: draft.title.trim() || '無題の問題', markdown: draft.markdown, editorial: draft.editorial,
