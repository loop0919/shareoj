...
useSharePreview({ title: () => problem.value!.title, path: problemPath, description: () => problem.value!.markdown.slice(0, 160), enabled: () => !!problem.value && !!contest.value && (contest.value.status === 'running' || contest.value.status === 'ended') })
</script>
<template>
  <div v-if="problem && contest" class="problem-page">
    <div class="breadcrumb-row"><nav class="breadcrumb" aria-label="パンくずリスト"><NuxtLink :to="`/contests/${id}?view=problems`">{{ contest.title }}</NuxtLink><span aria-hidden="true">/</span><span>{{ problem.title }}</span></nav><TweetButton v-if="contest.status === 'running' || contest.status === 'ended'" :title="problem.title" :url="problemPath" /></div>
