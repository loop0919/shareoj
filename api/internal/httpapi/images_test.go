...
func testContentImages(t *testing.T, request func(string, string, string, string) *httptest.ResponseRecorder, store *problems.Store) {
	...
	// Descriptions and problem images in drafts never become public on a timer.
	exec(`INSERT INTO contests(id,owner_id,title,description,starts_at,ends_at) VALUES($1,'alice','draft',$2,now()-interval '2 hours',now()-interval '1 hour')`, contestID, url)
	check("GET", "/images/"+id, "", "", 404)
	check("GET", "/my/images/"+id, "alice", "", 200)
	exec(`INSERT INTO problem_drafts(id,owner_id,draft) VALUES($1,'alice','{}')`, problemID)
	exec(`INSERT INTO contest_problems(contest_id,problem_id,position,points,draft,problem_version) VALUES($1,$2,0,100,jsonb_build_object('markdown',$3::text,'editorial',$3::text),1)`, contestID, problemID, url)
	check("GET", "/images/"+id, "", "", 404)
	...
	exec(`INSERT INTO contests(id,owner_id,title,description,starts_at,ends_at,published) VALUES($1,'alice','test','',now()+interval '1 hour',now()+interval '2 hours',true)`, contestID)
